...
contract Vesting is AccessControl {
    ...
    bytes32 internal constant VESTING_CONTROLLER_ROLE = keccak256("VESTING_CONTROLLER_ROLE");
    ...
    function vestTokens(
        ...
        require(beneficiary != address(0), "E1101");
        require(vestingSchedules[beneficiary].totalAllocation == 0, "E1102");
        require(totalAllocation > 0, "E1103");
        require(start > block.timestamp, "E1104");
        require(duration > 0, "E1105");
        require(duration > cliffDuration, "E1106");
    ...
    function revoke(address account) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(schedule.totalAllocation > 0, "E1107");

        uint256 remainingAmount = schedule.totalAllocation - schedule.released;
        require(remainingAmount > 0, "E1108");
    ...
    /// @notice Calculates the amount of released tokens
    /// @return The total amount of tokens released by contract
    ...
    function lockedAmount(address account) external view returns (uint256) {
        return vestingSchedules[account].totalAllocation - vestedAmount(account);
    }

    /// @notice Calculates the amount of releasable tokens for `account`
    /// @param account The address to check
    /// @return The amount of releasable tokens for `account`
    function releasableAmount(address account) public view virtual returns (uint256) {
    ...
    function _vestingSchedule(
        ...
        if (schedule.duration == 0 || timestamp < schedule.start + schedule.cliffDuration) {
            ...
            return (schedule.totalAllocation * (timestamp - schedule.start)) / schedule.duration;
        }
    }
}
