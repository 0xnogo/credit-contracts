...
export const checkAlmostSame = (a: BigNumberish, b: BigNumberish) => {
  expect(ethers.BigNumber.from(a).gte(ethers.BigNumber.from(b).mul(98).div(100))).to.be.true;
  expect(ethers.BigNumber.from(a).lte(ethers.BigNumber.from(b).mul(102).div(100))).to.be.true;
};
