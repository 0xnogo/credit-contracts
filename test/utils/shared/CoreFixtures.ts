...
import { BorrowParams, Lend, MintParams, PayParams } from "../../testCases/core";
...
import { getBlock, now } from "./Helper";
...
export async function lendFixture(fixture: Fixture, signer: SignerWithAddress, lendParam: Lend): Promise<any> {
  ...
  try {
    ...
    LendMath.getLoanInterest(pair.maturity, lendParam.lendInterestDecrease, await now());
    LendMath.getCoveragePrincipal(pairContractState, lendParam.lendAssetIn);
    LendMath.getCoverageInterest(pair.maturity, lendParam.lendCdpDecrease, await now());
...
export async function payFixture(fixture: Fixture, signer: SignerWithAddress, payParams: PayParams): Promise<Fixture> {
  const { pair, pairSim, assetToken, collateralToken } = fixture;
  const txn = await pair.upgrade(signer).pay(payParams.id, payParams.debtIn, payParams.collateralOut);
  const block = await getBlock(txn.blockHash!);
  pairSim.pay(
    pair.maturity,
    signer.address,
...
export async function withdrawFixture(
  ...
  const txnWithdraw = await pair
    .upgrade(signer)
    .withdraw(
      withdrawParams.loanPrincipal,
      withdrawParams.loanInterest,
      withdrawParams.coveragePrincipal,
      withdrawParams.coverageInterest
    );
