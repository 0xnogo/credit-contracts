...
export function pseudoRandomBigUint(maxUint: BigNumber): bigint {
  return BigInt(ethers.BigNumber.from(ethers.utils.randomBytes(32)).mod(maxUint).toString());
}

export function pseudoRandomBigUint256() {
  return BigInt(ethers.BigNumber.from(ethers.utils.randomBytes(32)).toString());
}
